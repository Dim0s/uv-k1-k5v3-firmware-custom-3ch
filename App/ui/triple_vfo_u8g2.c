...
static void apply_text_rect(uint8_t (*buffer)[128], uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, bool set_black)
{
...
void TripleVfoU8g2_DrawSmallText(const char *text, uint8_t x_left, uint8_t y_top, bool set_black)
{
    ...
    u8g2_SetFont(&s_u8g2, u8g2_font_5_tr);
    u8g2_DrawStr(&s_u8g2, x_left, (uint8_t)(y_top + 5u), prepared_text);

    const uint8_t width = TripleVfoU8g2_GetSmallTextWidth(prepared_text);
    if (width == 0u)
        return;
    apply_text_rect(gFrameBuffer, x_left, y_top, (uint8_t)(x_left + width - 1u), (uint8_t)(y_top + 5u), set_black);
}
...
void TripleVfoU8g2_DrawSmallTextStatus(const char *text, uint8_t x_left, uint8_t y_top, bool set_black)
{
    ...
    if (width == 0u)
        return;
    apply_text_rect(&gStatusLine, x_left, y_top, (uint8_t)(x_left + width - 1u), (uint8_t)(y_top + 5u), set_black);
}
